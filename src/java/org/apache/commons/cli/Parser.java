...
//    private List<String> requiredOptions;
    ...
    private void processProperties(Properties properties)
 {
    ...
    {
    	int size = options.getRequiredOptions().size(); // TODO reconsider 'size' thing
//    	final List<String> requiredOptions = ;
    	
    	// return if all required options have been processed
        if (size <= 0) return;
		...
		for (String option : (List<String>)options.getRequiredOptions()) {
			buff.append(option);
		}
	...
	private void processOption(String arg, ListIterator iter)
		...
		if (opt.isRequired()) {
			options.getRequiredOptions().remove(opt.getKey());
		}
		...
		if (group != null) {
			if (group.isRequired()) {
				options.getRequiredOptions().remove(group);
			}
